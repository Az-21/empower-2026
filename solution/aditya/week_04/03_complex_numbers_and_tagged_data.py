...
def real_part(tagged_impedance: Impedance) -> float:
  ...
  raise ValueError("Unknown impedance type")
...
print(round(real_part(element_two_reading), 4))  # expect 3.5355
print(round(imag_part(element_two_reading), 4))  # expect 3.5355
...
# WRITTEN ANSWER: Tagged data helps when the same kind of information can be
# represented in different ways. Instead of writing conditional checks in many
# places, the representation is identified once by a tag and generic operations
# can work with the data through a common interface.

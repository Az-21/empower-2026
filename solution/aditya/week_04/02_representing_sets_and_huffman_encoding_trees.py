...
def union_set(first_alert_set: Sequence[str], second_alert_set: Sequence[str]) -> tuple[str, ...]:
  ...
  while i < len(first_alert_set) and j < len(second_alert_set):
    if first_alert_set[i] == second_alert_set[j]:
    ...
    elif first_alert_set[i] < second_alert_set[j]:
      result.append(first_alert_set[i])
...
def make_leaf_set(symbol_weight_pairs: Sequence[tuple[str, int]]) -> tuple[HuffmanLeaf, ...]:
  ...
  for symbol, w in symbol_weight_pairs:
    result = adjoin_leaf_set(make_leaf(symbol, w), result)
...
cluster_a_alerts: tuple[str, ...] = ("LOW_BATTERY", "OFFLINE", "TEMP_HIGH")

cluster_b_alerts: tuple[str, ...] = ("OFFLINE", "TEMP_HIGH", "VIBRATION")

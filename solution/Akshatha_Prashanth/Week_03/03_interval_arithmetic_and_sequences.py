...
class Interval:
  ...
  """ frozen means read-only, using slots, python reserves memory only for the defined variables
...
def parallel_resistance(resistors: list[Interval]) -> Interval:
  ...
  if not resistors:
    raise ValueError("List of resistors cannot be empty.")

  reciprocals = []
  for resistor in resistors:
    if lower_bound(resistor) <= 0:
      raise ValueError("Resistor interval must be positive.")
    reciprocals.append(divide_interval(make_interval(1, 1), resistor))

  sum_reciprocal = reduce(add_interval, reciprocals)
  return divide_interval(make_interval(1, 1), sum_reciprocal)

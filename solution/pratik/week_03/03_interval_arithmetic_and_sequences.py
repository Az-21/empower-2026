...
def add_interval(first_interval: Interval, second_interval: Interval) -> Interval:
  return make_interval(lower_bound(first_interval) + lower_bound(second_interval),
...
def multiply_interval(first_interval: Interval, second_interval: Interval) -> Interval:
  ...
  return make_interval(
    min(lower_bound(first_interval) * lower_bound(second_interval),
        lower_bound(first_interval) * upper_bound(second_interval),
        upper_bound(first_interval) * lower_bound(second_interval),
        upper_bound(first_interval) * upper_bound(second_interval)),
    max(lower_bound(first_interval) * lower_bound(second_interval),
        lower_bound(first_interval) * upper_bound(second_interval),
        upper_bound(first_interval) * lower_bound(second_interval),
        upper_bound(first_interval) * upper_bound(second_interval)))
...
def divide_interval(first_interval: Interval, second_interval: Interval) -> Interval:
  ...
  # return multiply_interval(first_interval, make_interval(1 / upper_bound(second_interval), 1 / lower_bound(second_interval)))
...
def parallel_resistance(resistors: list[Interval]) -> Interval:
  ...
  reciprocals = [divide_interval(make_interval(1, 1), resistor)
                  for resistor in resistors]
  total_reciprocal = reduce(add_interval, reciprocals)
  return divide_interval(make_interval(1, 1), total_reciprocal)
